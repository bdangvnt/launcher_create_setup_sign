...
CALL set WinVer=10
CALL set buildNumberAndDate=b4619092025p
...
for %%w in (Win10) do (
    ...
    if not exist "%outputDirectory%TieuNgaoGiangHo_VTC_Setup.exe" (
        ...
        REM CALL:ECHORED "CALL iscc .\TNGH_Setup_Alpha.iss /DBuildSourcePath=%buildPath%\%%w\tngh /DoutputDirectory=%outputDirectory% /DOutputSetupFilename=TieuNgaoGiangHo_VTC_Setup /DrsBP=%rsBPB% /DeventTypePrefix=TNGH_B2C_%buildVersion%%w%%buildNumberAndDate%"
        CALL iscc .\TNGH_Setup_Alpha.iss /DBuildSourcePath=%buildPath%\%%w\tngh /DoutputDirectory=%outputDirectory% /DOutputSetupFilename=TieuNgaoGiangHo_VTC_Setup /DrsBP=%rsBPB% /DeventTypePrefix=TNGH_B2C_%buildVersion%%w%%buildNumberAndDate%
    ) else (
    ...
    for %%x in (fbppe01 fbppe02) do (
        if not exist "%outputDirectory%TieuNgaoGiangHo_VTC_Setup_%%x.exe" (
            CALL:ECHORED TNGH_%%x_%buildVersion%%w%%buildNumberAndDate%
            REM CALL:ECHORED "CALL iscc .\TNGH_Setup_Alpha.iss /DBuildSourcePath=%buildPath%\%%w\tngh_%%x /DoutputDirectory=%outputDirectory% /DOutputSetupFilename=TieuNgaoGiangHo_VTC_Setup_%%x /DrsBP=%rsBPB% /DeventTypePrefix=TNGH_%%x_%buildVersion%%w%%buildNumberAndDate%"
            CALL iscc .\TNGH_Setup_Alpha.iss /DBuildSourcePath=%buildPath%\%%w\tngh_%%x /DoutputDirectory=%outputDirectory% /DOutputSetupFilename=TieuNgaoGiangHo_VTC_Setup_%%x /DrsBP=%rsBPB% /DeventTypePrefix=TNGH_%%x_%buildVersion%%w%%buildNumberAndDate%
        ) else (
            CALL:ECHORED TNGH_%%x_%buildVersion%%w%buildNumberAndDate%
