...
CALL set buildNumberAndDate=b2030112023a
for %%w in (Win10) do (
	CALL:ECHORED %%w
	if not exist "..\..\..\..\launcher_create_setup_sign\inno\au2pc-setup-output-files-alpha\VTCPlus_%%w_Setup.exe" (
		CALL:ECHORED VTCPLUS_%%w_%buildVersion%_B2C_%buildNumberAndDate%
		CALL iscc .\VTCPlus_Setup_Alpha.iss /DBuildSourcePath=%buildPath%\%%w\au2pc /DoutputDirectory=%outputDirectory% /DOutputSetupFilename=VTCPlus_%%w_Setup /DrsBP=%rsBPB% /DeventTypePrefix=VTCPLUS_%%w_%buildVersion%_B2C_%buildNumberAndDate%
	)
	...
	for %%x in (baoch cococ fbreg fbscl ggdpl ggreg koccc kolll oohhh tikok vtcme ccoc1 ccoc2 ccoc3 ccoc4 ccoc5 vtcg1 vtcg2 vtcg3 vtcg4 vtcg5) do (
		...
		if not exist "..\..\..\..\launcher_create_setup_sign\inno\au2pc-setup-output-files-alpha\AU2PC_%%w_%%x_Setup.exe" (
			CALL:ECHORED %%x
			CALL del %outputDirectory%VTCPlus_%%w_%%x_Setup /F /Q
			CALL iscc .\VTCPlus_Setup_Alpha.iss /DBuildSourcePath=%buildPath%\%%w\AU2PC_%%x /DoutputDirectory=%outputDirectory% /DOutputSetupFilename=VTCPlus_%%w_%%x_Setup /DrsBP=%rsBPB% /DeventTypePrefix=VTCPLUS_%%w_%buildVersion%_%%x_%buildNumberAndDate%
		) else (
